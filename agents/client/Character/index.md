...
`Afterimage` 按当前攻击的精确姿态选择 Character/Skill NX 残影；武器等级超过素材最高档时，回退到仍包含该姿态的最近低档，不会用其他动作掩盖素材缺口。地图进入安全门会预热当前武器全部可能的普通攻击姿态、装备图层和残影。运行时残影走瞬态优先加载，不冻结玩法；触发帧先锁存，单帧动画在开始播放的首个更新周期不推进，保证至少经过一次绘制。本地玩家与其他角色均经 `Char` 共用这套生命周期。
