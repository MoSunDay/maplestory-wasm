...
`Afterimage` 按当前攻击的精确姿态选择 Character/Skill NX 残影；武器等级超过素材最高档时，回退到仍包含该姿态的最近低档，不会用其他动作掩盖素材缺口。残影触发帧会先锁存，冷缓存素材尚未驻留时不消耗动画时间；素材到达后即使角色已经恢复站立也会补播。一次攻击包含多个数字触发节点时，各节点独立播放。本地玩家与其他角色均经 `Char` 共用这套生命周期。
