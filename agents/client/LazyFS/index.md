...
6. 短时特效和动态掉落物通过 `requestPriorityFileRange()` 将已有预取提升到队首，但成功路径不登记前台阻塞、不显示遮罩；若优先请求失败，则升级到既有前台失败/重试流程，不静默降级
7. 地图进入期间始终显示带进度的阻塞遮罩，失败后停留并允许重试；正常游戏中只有显式阻塞可见素材会冻结模拟，纯后台预取和瞬态优先请求不显示遮罩
