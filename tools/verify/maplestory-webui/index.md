...
设置 `E2E_CAPTURE_ATTACK_EFFECTS=1` 会在进入地图后使用普通攻击，通过 CDP screencast 连续保存真实合成帧，并断言攻击期间没有素材遮罩或全局资源阻塞；默认采集 12 次，可用 `E2E_ATTACK_SAMPLES` 调整。它可以与 `E2E_STOP_AT_GAME=1` 组合，用于冷缓存和热缓存的武器残影验收。脚本只将该步骤标记为 `CAPTURE`，最终必须人工查看帧序列，确认剑光、命中反馈和怪物移动后才能判定视觉通过。
