...
namespace
{
        ...
        uint16_t job, bool sp_table)
    {
        ...
        append_number<int64_t>(bytes, 0);
        append_number<int64_t>(bytes, 0);
        append_number<uint16_t>(bytes, 42);
    ...
    jrc::CharEntry parse_fixture(const std::string& name, uint16_t job,
        bool sp_table)
    {
        Bytes bytes;
        append_stats(bytes, name, job, sp_table);
...
int main()
{
    ...
    const auto character = parse_fixture(chinese_name, 112, false);
    ...
    const std::string evan_name = u8"伊凡";
    const auto evan = parse_fixture(evan_name, 2200, true);
    assert(evan.stats.name == evan_name);
